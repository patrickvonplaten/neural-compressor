...
class TextDataset(Dataset):
    def __init__(self, tokenizer, args, file_path='train', block_size=1024):
        ...
        cached_features_file = os.path.join("./dataset_cached", 
            'distilgpt2_cached_lm_' + str(block_size) + '_' + filename)
...
import onnxruntime
class ONNXRTGPT2Dataset(CalibrationDataReader):
    def __init__(self, args, tokenizer, model_path):
        ...
        for idx, (inputs, labels) in enumerate(tqdm(eval_dataloader, desc="Evaluating")):
            inputs = inputs.to(args.device)
            labels = labels.to(args.device)
            self.dataset.append(np.expand_dims(np.array(inputs), axis=0))
            i -= 1
            if i == 0:
                break
...
def main():
    ...
    if args.tune:
        ...
        dr = ONNXRTGPT2Dataset(args, tokenizer, 'gpt2-optimized.onnx')
        from onnxruntime.quantization import quantize_static, QuantFormat
        quantize_static('gpt2-optimized.onnx',
                        args.output_model,
                        dr,
                        quant_format=QuantFormat.QOperator,
                        op_types_to_quantize=['Reshape', 'Slice', 'Unsqueeze', 'Gather', 'Add', 'Range', 'LayerNormalization', 
                                              'Shape', 'MatMul', 'Attention', 'Squeeze', 'FastGelu', 'Split', 'Transpose']
                        )
